...
int lua_execute (Byte *pc)//执行给定的操作码pc，返回0表示成功，返回1表示错误
{
 ...
 while (1)
 {
  ...
  switch (opcode = (OpCode)*pc++)
  {
   ...
   case FORPREP://专门为for循环设计的关键字，负责判断初始是否需要直接跳过
   {
    int      slot = *pc++;
  CodeWord skip_cw;
  get_word(skip_cw, pc);
  ...
  *(base + slot + 2) = step;
  ...
  // 根据 step 正负决定跳过条件
  if (nvalue(base + slot + 2) > 0)//step为正
  {
    if (nvalue(base + slot + 0) > nvalue(base + slot + 1))
      pc += skip_cw.w;
  }
  else//step为负
  {
  ...
  break;
   ...
   case FORLOOP://专门为for循环设计的关键字，负责执行局部变量i的变化，同时判断是否继续执行循环
   {
    int      slot = *pc++;
  CodeWord back_cw;
  get_word(back_cw, pc);
  ...
  //执行循环变量i的增加或减少
  nvalue(base + slot + 0) += nvalue(base + slot + 2);
